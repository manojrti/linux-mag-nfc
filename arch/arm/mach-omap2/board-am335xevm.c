...
#else /* EM2 */
	{"gpmc_ad5.gpio1_5", 0x07},
	{"gpmc_oen_ren.gpio2_3", 0x07},
	{"gpmc_wen.gpio2_4", 0x37},
	{"gpmc_wpn.gpio0_31", 0x2f},
#endif
};
...
static void trf7970a_setup(void)
{
#if 0 /* XXX EM1 */
	int gpio = GPIO_TO_PIN(1, 30);
#else /* EM2 */
	int gpio = GPIO_TO_PIN(1, 5);
#endif
	int status = gpio_request(gpio, "ss_gpio");
	if (status < 0)
		pr_err("Failed to request ss_gpio");
